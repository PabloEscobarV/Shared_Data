/***********************************************************************************************************************
 *  @file queue.hpp
 *  @brief Fixed-size circular queue template implementation.
 *
...
class FSQueue
{
  public:
     ...
     *
     *  @param swap_position Target position (0-based) where head element should be moved.
   ...
   private:
    uint16_t head;   ///< Index of the first element (next to be popped)
    uint16_t tail;   ///< Index where next element will be pushed
...
void FSQueue<data_t, size>::swap(uint16_t swap_position)
{
  ...
  // Perform swap only when there is at least one element and swap_position is within (0, count-1)
